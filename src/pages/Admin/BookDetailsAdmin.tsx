...
// function BookDetailsAdmin({ book }: IBookDetailsProps) {
...
import { useState } from "react";
import { createStyles, Table, ScrollArea, Button, Modal, Image } from "@mantine/core";
...
export function BookDetailsAdmin({ book }: IBookDetailsProps) {
  ...
  return (
    <tr>
      ...
      <td>{book.language}</td>
      <td>{book.price}</td>
      <td style={{ minWidth: "300px" }}>{book.description}</td>
      <td>
        <Modal opened={updated} onClose={() => setUpdated(false)} title="Edit book!">
          <UpdateBook book={book} />
        </Modal>
